...
```{r setup, include=FALSE}
library(jsonlite)
write(readLines ("drwho-210.json") |> prettify() , file = "json-pretty.txt")
episode_one <- drwho2023[[1]]
```


Is there any information stored in the list structure that you feel is redundant? If so, why?
Yes, the link to see the episode. Also, the information about each episode as the runtime, if it is regular or special, the season and number of episode. 
...
library(jsonlite)
library(purrr)
#Pass to data frame (convert to list and then use pluck for this)

files <- list(
  "1963" = "drwho-766.json",
  "2005" = "drwho-210.json",
  "2023" = "drwho-72724.json"
)

drwho_list <- map(files, read_json)

episodesinfo <- function(x, series_year) {
  map_dfr(x, function(ep) {
    data.frame(
      series_start = series_year,
      id = pluck(ep, "id", .default = NA),
      season = pluck(ep, "season", .default = NA),
      episode_number = pluck(ep, "number", .default = NA),
      runtime = pluck(ep, "runtime", .default = NA),
      rating = pluck(ep, "rating", "average", .default = NA),
      type = pluck(ep, "type", .default = NA)
    )
  })
}

drwho_episodes <- imap_dfr(drwho_list, episodesinfo)
...
I put the list once I was able to run it, so is different from the sketch 
---

1. Put json files in one file 
2. Use list function to convert the json files 
3. Use pluck to take the information of the episodes you want 
4. Use map_dfr to have all the information into a data frame 
